<?php
	...
	$get_grades="SELECT group_concat( a.grade 
		...
		for($x=0; $x<count($gradechunks); $x++){
			...
			$grades .='<td width = "45">'.$gradechunks[$x].'</td>';
		}
		$grades .= '</tr>';
		}
		...
		while($rows = mysql_fetch_array($get_ave_result)){
			...
			for($x=0; $x<$gradecount; $x++){
				$average = $average + $gradechunks[$x];
			}
				$average_result = number_format($average/$gradecount,2,'.','');
			if($average <= 60){
						$remarks = "RETAINED";
					} else{
						$remarks = "PROMOTED";
...
/*********************-END OF MALE-*********************/
	...
	$get_grades="SELECT group_concat( a.grade 
		...
		for($x=0; $x<count($gradechunks); $x++){
			...
			$fem_grades .='<td width = "45">'.$gradechunks[$x].'</td>';
		}
		$fem_grades .= '</tr>';
		}
